...
export const fetchRecipesCategory = async (category) => {
  try {
    const response = await axios.get(API_BASE_URL3, {
      params: { 
        apiKey: API_Key,   
        query: category,   // Pass the category here
        number: 10,        // Limit number of results (optional)
      },
    });
    return response.data.results; // Return the list of restaurant results (or you can modify based on response structure)
  } catch (error) {
